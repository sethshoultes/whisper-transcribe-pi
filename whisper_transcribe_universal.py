...
logging.basicConfig(
    ...
    datefmt='%H:%M:%S',
    handlers=[
        logging.FileHandler('/tmp/whisper_debug.log' if platform.system() != 'Windows' else 'whisper_debug.log'),
...
class UniversalClipboard:
    ...
    @staticmethod
    def copy(text):
        """Copy text to clipboard on any platform"""
...
class WorkingWhisper:
    ...
    def copy_all_transcriptions(self):
        ...
        def do_copy():
            ...
            if text:
                if UniversalClipboard.copy(text):
        ...
        with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as tmp:
            ...
            try:
                ...
                if text:
                    ...
                    # Copy to clipboard
                    clipboard_copied = UniversalClipboard.copy(text)
                    if clipboard_copied:
                    ...
                    self.ui_queue.put({
                        'type': 'transcription',
                        'text': display_text,
                        'status': f"✅ Transcribed #{self.transcription_count}" + (" (in clipboard)" if clipboard_copied else ""),
                        'transcription_count': self.transcription_count,
                        'actual_text': text  # Add for debugging
                    })
                    
                    # Save to file
                    history_file = os.path.expanduser("~/whisper_history.txt") if platform.system() != "Windows" else "whisper_history.txt"
...
def main():
    ...
    if system == "Linux":
        # Install xdotool if needed (Linux only)
        try:
            subprocess.run(['which', 'xclip'], check=True, capture_output=True)
        except:
            print("Note: Install xclip for clipboard support (sudo apt install xclip)")
    elif system == "Darwin":
        print("Running on macOS - clipboard support via pbcopy")
    elif system == "Windows":
        print("Running on Windows - clipboard support via clip")
